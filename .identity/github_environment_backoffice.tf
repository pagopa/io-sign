resource "github_repository_environment" "backoffice_app" {
  environment = "${local.project}-backoffice-app"
...
resource "github_actions_environment_secret" "backoffice_app_azure_subscription_id" {
  repository      = var.github.repository
  environment     = github_repository_environment.backoffice_app.environment
...
resource "github_actions_environment_variable" "backoffice_app_azure_web_app_rg" {
  repository    = var.github.repository
  environment   = github_repository_environment.backoffice_app.environment
  variable_name = "AZURE_WEB_APP_RESOURCE_GROUP"
  value         = "${local.project}-backend-rg"
}

resource "github_actions_environment_variable" "backoffice_app_azure_web_app_name" {
  repository    = var.github.repository
  environment   = github_repository_environment.backoffice_app.environment
  variable_name = "AZURE_WEB_APP_NAME"
  value         = "${local.project}-backoffice-app"
}
