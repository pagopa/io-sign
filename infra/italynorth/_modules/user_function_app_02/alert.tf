resource "azurerm_monitor_scheduled_query_rules_alert" "get_signature_request_not_found_02" {
  name                = "${local.project_itn_sign}-user-func-02-get-signature-request-not-found"
  resource_group_name = data.azurerm_resource_group.sign_itn_rg.name
  location            = local.location_itn

  data_source_id          = data.azurerm_application_insights.application_insights.id
  description             = "[IO-SIGN] getSignatureRequest returns 404 on io-p-itn-sign-user-func-02"
  enabled                 = true
  auto_mitigation_enabled = false

  query = <<-QUERY
requests
| where cloud_RoleName == "io-p-itn-sign-user-func-02"
