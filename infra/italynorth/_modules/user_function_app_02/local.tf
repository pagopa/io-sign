locals {
  ...
  io_sign_user_func_02 = {
    app_settings = {
      ...
      LollipopExternalApiBasePath       = "https://api.io.pagopa.it"
      LollipopExternalApiKey            = "@Microsoft.KeyVault(VaultName=${data.azurerm_key_vault.sign_kv.name};SecretName=LollipopExternalPrimaryApiKey)"
      ...
      IoProfileApiBasePath              = "@Microsoft.KeyVault(VaultName=${data.azurerm_key_vault.sign_kv.name};SecretName=IoProfileApiBasePath)"
      IoProfileSubscriptionKey          = "@Microsoft.KeyVault(VaultName=${data.azurerm_key_vault.sign_kv.name};SecretName=IoProfileSubscriptionKey)"
      LollipopInternalApiBasePath       = "@Microsoft.KeyVault(VaultName=${data.azurerm_key_vault.sign_kv.name};SecretName=LollipopInternalApiBasePath)"
      LollipopInternalApiKey            = "@Microsoft.KeyVault(VaultName=${data.azurerm_key_vault.sign_kv.name};SecretName=LollipopInternalApiKey)"
