locals {
  ...
  io_sign_backoffice_func = {
    app_settings = {
      ...
      SLACK_WEBHOOK_URL                 = "@Microsoft.KeyVault(VaultName=${data.azurerm_key_vault.sign_weu_kv.name};SecretName=slack-webhook-url)"
      BACK_OFFICE_API_KEY               = "@Microsoft.KeyVault(VaultName=${data.azurerm_key_vault.sign_weu_kv.name};SecretName=BackOfficeApiKey)"
      GOOGLE_PRIVATE_KEY                = "@Microsoft.KeyVault(VaultName=${data.azurerm_key_vault.sign_weu_kv.name};SecretName=bo-google-private-key)"
      GOOGLE_CLIENT_EMAIL               = "@Microsoft.KeyVault(VaultName=${data.azurerm_key_vault.sign_weu_kv.name};SecretName=bo-google-client-email)"
      GOOGLE_SPREADSHEET_ID             = "@Microsoft.KeyVault(VaultName=${data.azurerm_key_vault.sign_weu_kv.name};SecretName=bo-google-spreadsheet-id)"
