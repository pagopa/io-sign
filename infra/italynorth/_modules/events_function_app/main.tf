module "function_sign_events" {
  ...
  subnet_cidr                              = var.sign_events_snet_cidr
  health_check_path                        = "/api/v1/sign/events/info"
...
module "itn_sign_events_func_roles" {
  source          = "pagopa-dx/azure-role-assignments/azurerm"
  version         = "~> 1.2.0"
  principal_id    = module.function_sign_events.function_app.function_app.principal_id
  ...
  key_vault = [
    {
      ...
      description         = "Allow ${module.function_sign_events.function_app.function_app.name} to read secrets from ${data.azurerm_key_vault.sign_kv.name}"
...
module "itn_sign_events_func_staging_roles" {
  source          = "pagopa-dx/azure-role-assignments/azurerm"
  version         = "~> 1.2.0"
  principal_id    = module.function_sign_events.function_app.function_app.slot.principal_id
  subscription_id = data.azurerm_subscription.current.subscription_id
  ...
  key_vault = [
    {
      name                = data.azurerm_key_vault.sign_kv.name
      resource_group_name = data.azurerm_key_vault.sign_kv.resource_group_name
      description         = "Allow ${module.function_sign_events.function_app.function_app.slot.name} to read secrets from ${data.azurerm_key_vault.sign_kv.name}"
      has_rbac_support    = true
      roles = {
        secrets = "reader"
      }
