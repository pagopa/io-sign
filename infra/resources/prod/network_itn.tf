...
resource "azurerm_subnet_nat_gateway_association" "io_sign_user_snet_itn" {
  #TODO use nat gateway in itn
  nat_gateway_id = data.azurerm_nat_gateway.nat_gateway_itn.id
  subnet_id      = module.io_sign_user_snet_itn.id
}

resource "azurerm_private_endpoint" "io_sign_user_func_itn" {
  name                = format("%s-user-func-pep-01", local.project_itn_sign)
  location            = azurerm_resource_group.data_rg_itn.location
  resource_group_name = azurerm_resource_group.data_rg_itn.name
  subnet_id           = data.azurerm_subnet.itn_private_endpoints_subnet.id
  ...
  private_service_connection {
    name                           = format("%s-user-func-pep-01", local.project_itn_sign)
    private_connection_resource_id = module.io_sign_user_func_itn.id
    is_manual_connection           = false
    subresource_names              = ["sites"]
  }
...
resource "azurerm_private_endpoint" "io_sign_user_func_staging_itn" {
  count = var.io_sign_user_func.sku_tier == "PremiumV3" ? 1 : 0

  name                = format("%s-user-func-staging-pep-01", local.project_itn_sign)
  location            = azurerm_resource_group.data_rg_itn.location
  resource_group_name = azurerm_resource_group.data_rg_itn.name
  subnet_id           = data.azurerm_subnet.itn_private_endpoints_subnet.id
...
resource "azurerm_subnet_nat_gateway_association" "io_sign_support_snet_itn" {
  nat_gateway_id = data.azurerm_nat_gateway.nat_gateway_itn.id
  subnet_id      = module.io_sign_support_snet_itn.id
}

resource "azurerm_private_endpoint" "io_sign_support_func_itn" {
  name                = format("%s-support-func-pep-01", local.project_itn_sign)
  location            = azurerm_resource_group.data_rg_itn.location
  resource_group_name = azurerm_resource_group.data_rg_itn.name
  subnet_id           = data.azurerm_subnet.itn_private_endpoints_subnet.id
  ...
  private_service_connection {
    name                           = format("%s-support-func-pep-01", local.project_itn_sign)
    private_connection_resource_id = module.io_sign_support_func_itn.id
    is_manual_connection           = false
    subresource_names              = ["sites"]
  }
...
resource "azurerm_private_endpoint" "io_sign_support_func_staging_itn" {
  count = var.io_sign_support_func.sku_tier == "PremiumV3" ? 1 : 0

  name                = format("%s-support-func-staging-pep-01", local.project_itn_sign)
  location            = azurerm_resource_group.data_rg_itn.location
  resource_group_name = azurerm_resource_group.data_rg_itn.name
  subnet_id           = data.azurerm_subnet.itn_private_endpoints_subnet.id
...
resource "azurerm_private_endpoint" "io_sign_backoffice_func_staging_itn" {

  name                = format("%s-backoffice-func-staging-pep-01", local.project_itn_sign)
  location            = azurerm_resource_group.data_rg_itn.location
  resource_group_name = azurerm_resource_group.data_rg_itn.name
  subnet_id           = data.azurerm_subnet.itn_private_endpoints_subnet.id
