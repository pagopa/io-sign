import {
  ClauseTypeEnum,
  Configuration, Document, DocumentReady, DocumentToBeUploaded, SignatureRequestApi} from "@io-sign/io-sign-api-client";
...
vi.mock("./upload-file");
const mockGetSignatureRequest = vi.spyOn(
  SignatureRequestApi.prototype,
  "getSignatureRequest"
).mockImplementation(() => { return Promise.resolve(getSignatureResponse) } );
const mockCreateSignatureRequest = vi.spyOn(
  SignatureRequestApi.prototype,
  "createSignatureRequest"
).mockImplementation(() => { return Promise.resolve(getSignatureResponse) } );
const mockGetDocumentUploadUrl = vi.spyOn(
  SignatureRequestApi.prototype,
  "getDocumentUploadUrl"
).mockImplementation(() => { return Promise.resolve("www.example.com") });
...
//    mockCallDocumentUpload.mockClear();
  });
...
return Promise.resolve(getSignatureRequestWithDocumentsUploadedResponse);
  ...
  mockGetSignatureRequest.mockImplementation(() => {
return Promise.resolve(getSignatureRequestWithWaitForSignatureStatusResponse);
  });
    await callSignatureRequests({} as Configuration, request as any);
    expect(mockGetSignatureRequest).toBeCalledTimes(3); // it should be 8 times
    expect(mockCreateSignatureRequest).not.toBeCalled;
  ...
  /*
  test("makes a call to upload the documents", async () => {
    const request = { 
      signatureRequest : { 
        id: "AAA",
    },
...
//    expect(mockGetSignatureRequest).toBeCalledTimes(3); // it should be 8 times
    expect(mockCreateSignatureRequest).not.toBeCalled;
    expect(mockGetDocumentUploadUrl).toBeCalled;
    expect(mockSendNotification).not.toBeCalled;
    expect(callDocumentUpload).toBeCalled;
  });
  */
});
